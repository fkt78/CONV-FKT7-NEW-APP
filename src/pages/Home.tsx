...
export default function Home() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      // チャットルーム親ドキュメントを upsert（初回送信時に自動作成）
      await setDoc(
  ...
  return (
    <div className="h-dvh bg-[#0f0f23] flex flex-col max-w-lg mx-auto">
      ...
      {/* ── プロフィールカード（コンパクト） ── */}
      <div className="mx-4 mt-3 rounded-xl bg-gradient-to-br from-[#1a1a2e] to-[#16213e] border border-amber-400/10 p-3 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
            ...
            {userData?.birthMonth && (
              <span className="text-[10px] text-white/30">{userData.birthMonth}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pt-3 pb-1.5 flex-shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded-full bg-gradient-to-br from-amber-400 to-yellow-600 flex items-center justify-center flex-shrink-0">
